...
def register_user_view(request):
    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            next_page, form, error_message = UserRegistrationService.registration(
                request=request
            )
            ...
            if error_message or form.errors:
                if next_page:
                    if "order" in next_page:
                        return render(
                            request,
                            "app_shop/orders/registration/order.html",
                            {"form": form, "error_message": error_message},
                        )

                return render(
                    request,
                    "app_user/registration.html",
                    {"form": form, "error_message": error_message},
                )
...
class LoginUserView(FormView):
    ...
    def form_valid(self, form, error_message=""):
        ...
        next_page, account, error_message = UserRegistrationService.login(
        ...
        if next_page:
            ...
            return render(
                self.request,
                "../templates/app_user/account/login.html",
                {"form": form, "error_message": error_message},
            )
...
class PasswordRecoveryView(FormView):
    ...
    def form_valid(self, form, message="", error_message=""):
        ...
        return render(
            self.request,
            "../templates/app_user/account/password_recovery.html",
            {
                "form": form,
                "message": message,
...
def account_view(request):
    ...
    if request.user.is_authenticated:
        ...
        return render(
            request,
            "../templates/app_user/account/account.html",
...
class ProfileView(View):
    ...
    def get(self, request):
        ...
        if request.user.is_authenticated:
            form = RegisterUserForm()
            return render(
                request, "../templates/app_user/account/profile.html", {"form": form}
            )
...
class UpdateAccountView(FormView):
    ...
    def form_valid(self, form, message="", error_message=""):
        logger.debug(f"Данные валидны: {form.cleaned_data}")
        message, error_message = UserRegistrationService.editing_data(
            request=self.request, form=form
        )

        return render(
            self.request,
            "../templates/app_user/account/profile.html",
            {
                "form": form,
                "message": message,
                "error_message": error_message,
