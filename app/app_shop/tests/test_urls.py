...
class TestUrls(TestCase):
    ...
    def setUpTestData(cls):
        ...
        test_user = User.objects.create(username='test_user', password='secret_password')
        test_img = open(os.path.join('app_shop', 'tests', 'test_img.jpg'), 'rb').read()

        test_category = CategoryProduct.objects.create(
            title='Тестовая категория',
            image=SimpleUploadedFile(name='test_img.jpg', content=test_img, content_type='image/jpeg')
        )
